...
export const computeHover = (drag, hover, actions, { room, mouse, matrix, callbacks, }, m
 ...
 */
export const computeLevel = ({ size, levels, position, }) => {
    if (size <= (levels + 1) * 2) {
...
const getDropLevel = (hover) => (hover.node.inline ? 1 : 0);
/**
 * A list of callbacks.
 */
export const defaultCallbacks = {
    [c.NO]: (item, hover, { clear }) => clear(item.id),
    /* corners */
    [c.C1]: (item, hover, { leftOf, above }, 
    // tslint:disable-next-line:no-any
    ctx) => {
    ...
    /* heres */
    [c.AH]: (item, hover, { above }) => {
        const level = getDropLevel(hover);
        above(item.rawNode(), Object.assign({}, hover.rawNode()), level);
    },
    [c.BH]: (item, hover, { below }) => {
        const level = getDropLevel(hover);
        below(item.rawNode(), Object.assign({}, hover.rawNode()), level);
    },
    [c.LH]: (item, hover, { leftOf }) => {
        const level = getDropLevel(hover);
        leftOf(item.rawNode(), Object.assign({}, hover.rawNode()), level);
    },
    [c.RH]: (item, hover, { rightOf }) => {
        const level = getDropLevel(hover);
        rightOf(item.rawNode(), Object.assign({}, hover.rawNode()), level);
    },
    /* ancestors */
    [c.AA]: (item, hover, { above }, ctx) => above(item.rawNode(), hover.rawNode(), computeVertical(Object.assign({}, ctx, { hover: hover, level: hover.node.levels.above }), true)),
    [c.BA]: (item, hover, { below }, ctx) => below(item.rawNode(), hover.rawNode(), computeVertical(Object.assign({}, ctx, { hover, level: hover.node.levels.below }))),
    [c.LA]: (item, hover, { leftOf }, ctx) => leftOf(item.rawNode(), hover.rawNode(), computeHorizontal(Object.assign({}, ctx, { hover, level: hover.node.levels.left }), true)),
    [c.RA]: (item, hover, { rightOf }, ctx) => rightOf(item.rawNode(), hover.rawNode(), computeHorizontal(Object.assign({}, ctx, { hover, level: hover.node.levels.right }))),
    /* inline */
...
export default class HoverService {
    constructor({ matrices, callbacks } = {}) {
    ...
    hover(drag, hover, actions, { room, mouse, matrix: use = '10x10', }) {
